...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, updateEntity, partialUpdateEntity, reset } from './show-seat.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_SHOWSEAT_LIST actions', async () => {
    ...
    it('dispatches FETCH_SHOWSEAT actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_SHOWSEAT actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ showSeatId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_SHOWSEAT actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ showSeatId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_SHOWSEAT actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ showSeatId: 123 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_SHOWSEAT actions', async () => {
      const expectedActions = [
        {
