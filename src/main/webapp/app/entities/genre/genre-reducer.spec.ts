...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, updateEntity, partialUpdateEntity, reset } from './genre.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_GENRE_LIST actions', async () => {
    ...
    it('dispatches FETCH_GENRE actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_GENRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ genreId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_GENRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ genreId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_GENRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ genreId: 123 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_GENRE actions', async () => {
      const expectedActions = [
        {
