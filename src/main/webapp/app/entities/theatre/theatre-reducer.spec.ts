...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, updateEntity, partialUpdateEntity, reset } from './theatre.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_THEATRE_LIST actions', async () => {
    ...
    it('dispatches FETCH_THEATRE actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_THEATRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ theatreId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_THEATRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ theatreId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_THEATRE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ theatreId: 123 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_THEATRE actions', async () => {
      const expectedActions = [
        {
