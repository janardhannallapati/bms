...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, updateEntity, partialUpdateEntity, reset } from './show.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_SHOW_LIST actions', async () => {
    ...
    it('dispatches FETCH_SHOW actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_SHOW actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ showId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_SHOW actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ showId: 456 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_SHOW actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ showId: 123 }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_SHOW actions', async () => {
      const expectedActions = [
        {
