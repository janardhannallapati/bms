...
import com.jana.bms.service.PaymentQueryService;
import com.jana.bms.service.PaymentService;
...
/**
 * REST controller for managing {@link com.jana.bms.domain.Payment}.
 */
@RestController
@RequestMapping("/api")
public class PaymentResource {
    ...
    public ResponseEntity<PaymentDTO> createPayment(@Valid @RequestBody PaymentDTO paymentDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            ...
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getPaymentId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /payments/:paymentId} : Updates an existing payment.
     *
     * @param paymentId the id of the paymentDTO to save.
    ...
    public ResponseEntity<PaymentDTO> updatePayment(
        @PathVariable(value = "paymentId", required = false) final Long paymentId,
        @Valid @RequestBody PaymentDTO paymentDTO
    ) throws URISyntaxException {
        log.debug("REST request to update Payment : {}, {}", paymentId, paymentDTO);
        if (paymentDTO.getPaymentId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(paymentId, paymentDTO.getPaymentId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, paymentDTO.getPaymentId().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /payments/:paymentId} : Partial updates given fields of an existing payment, field will ignore if it is null
     *
     * @param paymentId the id of the paymentDTO to save.
     * @param paymentDTO the paymentDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated paymentDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/payments/{paymentId}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<PaymentDTO> partialUpdatePayment(
        @PathVariable(value = "paymentId", required = false) final Long paymentId,
        @NotNull @RequestBody PaymentDTO paymentDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Payment partially : {}, {}", paymentId, paymentDTO);
        if (paymentDTO.getPaymentId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(paymentId, paymentDTO.getPaymentId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, paymentDTO.getPaymentId().toString())
        );
    }
    ...
    public ResponseEntity<List<PaymentDTO>> getAllPayments(
        ...
        log.debug("REST request to get Payments by criteria: {}", criteria.toString().replaceAll("[\n\r\t]", "_"));
    ...
    public ResponseEntity<Long> countPayments(PaymentCriteria criteria) {
        log.debug("REST request to count Payments by criteria: {}", criteria.toString().replaceAll("[\n\r\t]", "_"));
